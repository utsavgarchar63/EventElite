    ...
    <div class="d-flex justify-space-between align-center mb-3">
      <div class="d-flex align-center">
        ...
        <v-avatar size="64" class="mr-3">
          <!-- Show initials if user role -->
          <template v-if="userRole === 'user'">
            <v-avatar
              size="64"
              color="primary"
              class="d-flex align-center justify-center text-white text-h6 font-weight-medium"
            >
              {{ userInitials }}
            </v-avatar>
          </template>

          <!-- Show image for other roles -->
          <template v-else>
        ...
        <div>
          ...
          <p v-if="user.phone" class="text-body-2 text-grey-darken-1 mb-0">
            {{ user.phone }}
          </p>
        </div>
      </div>
    ...
    <v-row>
      ...
      <v-col cols="4" class="pa-1">
        <v-sheet class="pa-3 text-center rounded-md bg-amber-lighten-5 border">
          <h3 class="text-h6 font-weight-bold mb-1 text-amber-darken-2">
            ₹{{ user.totalSpent }}
          </h3>
...
// ✅ Role & user data from localStorage
const userRole = localStorage.getItem('role');
const storedUser = JSON.parse(localStorage.getItem('user') || '{}');

// ✅ Compute initials
const userInitials = computed(() => {
  const name = storedUser?.name || user.value.name || '';
