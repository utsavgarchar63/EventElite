    ...
    <div class="form-wrapper">
      <div class="mb-8">
        ...
        <p class="mt-1" style="color: #525454">
          Please provide the list of sponsors of the event.
        </p>
      </div>

      <v-form @submit.prevent="handleSubmit">
        <!-- Multi-select for existing sponsors -->
        <v-col cols="12" class="pa-2 mb-6">
          <v-select
            ...
            multiple
            chips
            label="Select Existing Sponsors"
            variant="outlined"
            @update:modelValue="handleSelectedSponsors"
          />
        </v-col>
        ...
        <v-row v-for="(sponsor, index) in sponsors" :key="index" class="mb-4">
          ...
          <v-col cols="12" md="6" class="pa-2">
            <v-text-field
              label="Business Name"
              v-model="sponsor.businessName"
              :readonly="sponsor.isSelected"
...
// our multi-select state
const selectedSponsors = ref([]);

// reactive sponsors form array
const sponsors = ref([]);

// init sponsors from store
const initSponsors = () => {
  if (store.formData.sponsors.length) {
    sponsors.value = store.formData.sponsors.map((s) => ({
      ...
      businessLink: s.link || s.businessLink || '',
      businessLogo: s.logo || s.businessLogo || null,
      isSelected: !!s.id,
    }));
...
// fill sponsor data when selected
const fillSponsorData = (id) => {
...
// when multi-select changes
const handleSelectedSponsors = (newVal) => {
  // add new selected sponsors
  newVal.forEach((id) => {
    const already = sponsors.value.find(
      (s) => s.selectedId === id && s.isSelected
    );
    if (!already) {
      const data = fillSponsorData(id);
      if (data) sponsors.value.push(data);
    }
  });
  // remove sponsors if unselected
  sponsors.value = sponsors.value.filter((s) =>
    s.isSelected ? newVal.includes(s.selectedId) : true
  );
};
...
const fetchSponsors = async () => {
  try {
    ...
    if (res.data.success) {
      ...
      await nextTick();
      // auto-fill any pre-selected
      selectedSponsors.value.forEach((id) => handleSelectedSponsors(selectedSponsors.value));
    }
  } catch (err) {
