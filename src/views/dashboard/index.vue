  ...
  <v-container>
    <v-row>
      ...
      <template v-if="role === 'super_admin'">
        <v-col v-for="card in superAdminCards" :key="card.title" cols="12" sm="6" md="3" style="padding: 0 7px;">
          <div>
            <MetricCard :title="card.title" :avatarColor="card.avatarColor" :value="card.value" :growth="card.growth"
              :icon="card.icon" />
          </div>
        </v-col>
              ...
              <v-form ref="formRef" @submit.prevent="handleSubmit">
                <v-row>
                  ...
                  <v-col cols="12">
                    ...
                    <v-text-field v-model="firstName" variant="outlined" hide-details="auto" color="primary"
                      placeholder="Enter first name" :error="!!errors.firstName" :error-messages="errors.firstName" />
                  </v-col>

                  <!-- Last Name -->
                  <v-col cols="12">
                    <v-label>Last Name</v-label>
                    <v-text-field v-model="lastName" variant="outlined" hide-details="auto" color="primary"
                      placeholder="Enter last name" :error="!!errors.lastName" :error-messages="errors.lastName" />
                  </v-col>
                  ...
                  <v-col cols="12">
                    ...
                    <v-text-field v-model="email" variant="outlined" hide-details="auto" color="primary"
                      placeholder="Enter email" :error="!!errors.email" :error-messages="errors.email" />
                  </v-col>

                  <!-- Phone -->
                  <v-col cols="12">
                    <v-label>Phone</v-label>
                    <v-text-field v-model="phone" variant="outlined" hide-details="auto" color="primary"
                      placeholder="Enter phone number" :error="!!errors.phone" :error-messages="errors.phone" />
                  </v-col>
                  ...
                  <v-col cols="12">
                    <v-select v-model="organizationId" :items="organizationList" item-title="legal_name" item-value="id"
                      label="Select Organization" variant="outlined" placeholder="Select an existing organization" />
                  </v-col>
                  <v-checkbox v-model="createNewOrg" label="Create a new organization" />


                  <v-col cols="12 " class="mb-5">
                    <v-text-field v-model="legalName" variant="outlined" hide-details="auto" color="primary"
        ...
        <!-- Dynamic Views -->
        <!-- <div v-if="currentView === 'organization'" style="margin-top: 20px; width: 100%;"> -->
        <Organizations />
        <!-- </div> -->

        <!-- <div v-else-if="currentView === 'events'" style="margin-top: 20px; width: 100%;">
          <Events />
        </div>

        <div v-else-if="currentView === 'membership'" style="margin-top: 20px; width: 100%;">
          <MembershipRevenue />
        </div>

        <div v-else-if="currentView === 'totalRevenue'" style="margin-top: 20px; width: 100%;">
          <TotalRevenue />
        </div> -->
      </template>
      ...
      <template v-else-if="role === 'admin'">
        <v-col style="padding: 0px 7px;" cols="12" sm="6" md="3" v-for="card in adminCards" :key="card.title">
          <MetricCard :title="card.title" :avatarColor="card.avatarColor" :value="card.value" :growth="card.growth"
            :icon="card.icon" :redirect="card.redirect" />
        </v-col>
        <!-- Charts for Admin -->
        ...
        <v-col cols="12" lg="7" style="margin-top: 20px;" v-for="event in onGoingEvents" :key="event.id">
          <v-card class="py-5 px-1" style="border-radius: 8px !important;">
            ...
            <v-data-table :headers="headers" class="custom-table" :items="onGoingEvents" hide-default-footer dense>
...
const currentView = ref<string | null>("organization");
...
//   if (cardTitle === "Total Organization") {
//     currentView.value = "organization";
//   } else if (cardTitle === "Total Events") {
//     currentView.value = "events";
//   } else if (cardTitle === "Membership Revenue") {
//     currentView.value = "membership";
//   } else if (cardTitle === "Total Revenue") {
//     currentView.value = "totalRevenue";
//   } else {
