  ...
  <v-container>
    <div style="background-color: white; padding: 10px; border-radius: 8px;">
      ...
      <div class="pagination-container">
        <v-pagination v-model="page" :length="pageCount" total-visible="5" @update:modelValue="fetchCancelledEvents" />
      </div>
    </div>
  </v-container>
</template>
...
// --- Fetch Cancelled Events (Server-side Pagination) ---
...
const fetchCancelledEvents = async () => {
  ...
  try {
    ...
    const response = await api.get(`/events/cancelled`, { params });
...
// --- On mounted ---
onMounted(() => fetchCancelledEvents());
</script>
<style scoped>
.header-section {
  display: flex;
